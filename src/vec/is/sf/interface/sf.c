...
PETSC_UNUSED static PetscErrorCode PetscSFCheckLeavesUnique_Private(PetscSF sf)
{
#if defined(PETSC_USE_DEBUG)
...
. sfBA - The composite SF
  ...
  Notes:
  Currently, the two SFs must be defined on congruent communicators.
  Doing a Bcast on the new SF is equivalent to doing a Bcast on sfA, then a Bcast on sfB.
  If the two SFs are true star forests, i.e. the same leaf is not connected with different roots,
  the above sequence of operations is well-defined and the resulting composed SF will be well-defined too.
  In the case of leaves connected with multiple roots in sfA only, the composed SF will be well-defined in the
  sense that calling twice this routine will create the same SF.
  Instead, when sfB is not a true star forest and the communicator size is larger than one,
  the computed SF may differ from one call to the other.
...
. sfBA - The composite SF.
  ...
  Notes:
  Currently, the two SFs must be defined on congruent communicators.
  Doing a Bcast on the new SF is equivalent to doing a Bcast on sfA, then a Bcast on inverse of sfB.
  For the composite SF to be well-defined, all roots of sfB must have degree 1 (i.e. the inverse SF
  should be well-defined). The same considerations described in PetscSFCompose() on sfA and sfB apply
  here, replacing sfB with its inverse SF.
