...
static PetscErrorCode PCSetUp_H2OPUS(PC pc)
{
  ...
  if (ish2opus) {
    ...
    const char *prefix;
    ierr = MatCreateH2OpusFromMat(A,pch2opus->sdim,pch2opus->coords,pch2opus->eta,pch2opus->leafsize,pch2opus->max_rank,pch2opus->bs,pch2opus->mrtol,&pch2opus->A);CHKERRQ(ierr);
    /* XXX */
    ...
    ierr = MatAppendOptionsPrefix(pch2opus->A,"pc_h2opus_inv_");CHKERRQ(ierr);
  ...
  if (initerr > pch2opus->atol && !pc->failedreason) {
    ...
    for (i = 0; i < pch2opus->maxits; i++) {
      ...
      ierr = MatDuplicate(pch2opus->A,MAT_SHARE_NONZERO_PATTERN,&M);CHKERRQ(ierr);
      ierr = MatGetOptionsPrefix(pch2opus->A,&prefix);CHKERRQ(ierr);
...
static PetscErrorCode PCView_H2OPUS(PC pc, PetscViewer viewer)
{
  ...
  if (isascii) {
    if (pch2opus->M) {
      ierr = PetscViewerASCIIPrintf(viewer,"Inner matrix used\n");CHKERRQ(ierr);
