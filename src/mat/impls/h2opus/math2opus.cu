...
template <class T> class PetscPointCloud : public H2OpusDataSet<T>
{
  ...
  public:
    ...
    PetscPointCloud(const PetscPointCloud<T>& other)
    {
      ...
      this->pts.resize(this->dimension);
...
PetscErrorCode VecSetDelta(Vec x, PetscInt i)
{
#if defined(PETSC_HAVE_CUDA)
  PetscBool      iscuda;
#endif
...
/* this is declared PETSC_EXTERN since it is used in pch2opus */
PETSC_EXTERN PetscErrorCode MatH2OpusSetNativeMult(Mat A, PetscBool nm)
{
  Mat_H2OPUS     *a = (Mat_H2OPUS*)A->data;
  PetscBool      ish2opus;
  PetscErrorCode ierr;
