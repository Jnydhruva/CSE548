...
#if defined(PETSC_HAVE_IMMINTRIN_H) && defined(__AVX512F__) && defined(PETSC_USE_REAL_DOUBLE) && !defined(PETSC_USE_COMPLEX) && !defined(PETSC_USE_64BIT_INDICES)
  if (a->sliceheight != 8) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_SUP, "The kernel requires a slice height of 8, but the input matrix has a slice height of %" PetscInt_FMT, a->sliceheight);
...
PetscErrorCode MatSeqSELLSetSliceHeight_SeqSELL(Mat A, PetscInt sliceheight)
{
  ...
  if (a->sliceheight > 0 && a->sliceheight != sliceheight) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_SUP, "Cannot change slice height %" PetscInt_FMT " to %" PetscInt_FMT, a->sliceheight, sliceheight);
  a->sliceheight = sliceheight;
#if defined(PETSC_HAVE_DEVICE)
  if (DEVICE_MEM_ALIGN % sliceheight) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_SUP, "DEVICE_MEM_ALIGN is not divisible by the slice height %" PetscInt_FMT, sliceheight);
#endif
  PetscFunctionReturn(0);
}
...
{
  PetscFunctionBegin;
  PetscCall(PetscUseMethod(A, "MatSeqSELLGetFillRatio_C", (Mat, PetscScalar *), (A, ratio)));
  PetscFunctionReturn(0);
}
...
{
  PetscFunctionBegin;
  PetscCall(PetscUseMethod(A, "MatSeqSELLGetMaxSliceWidth_C", (Mat, PetscInt *), (A, slicewidth)));
  PetscFunctionReturn(0);
}
...
{
  PetscFunctionBegin;
  PetscCall(PetscUseMethod(A, "MatSeqSELLGetAvgSliceWidth_C", (Mat, PetscReal *), (A, slicewidth)));
  PetscFunctionReturn(0);
}
...
{
  PetscFunctionBegin;
  PetscCall(PetscUseMethod(A, "MatSeqSELLSetSliceHeight_C", (Mat, PetscInt), (A, sliceheight)));
  PetscFunctionReturn(0);
}
...
PETSC_EXTERN PetscErrorCode MatCreate_SeqSELL(Mat B)
{
  ...
  PetscObjectOptionsBegin((PetscObject)B);
  {
    ...
    if (flg) {
      if (chunksize < 64 || chunksize > 1024 || chunksize % 64) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "chunksize must be a number in {64,128,256,512,1024}: value %" PetscInt_FMT, chunksize);
