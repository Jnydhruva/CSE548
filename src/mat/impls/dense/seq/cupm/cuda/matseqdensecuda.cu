...
using namespace Petsc::mat::cupm::impl;
using Petsc::device::cupm::DeviceType;

static constexpr MatSeqDense_CUPM<DeviceType::CUDA> cupm_mat{};

/*MC
  MATSEQDENSECUDA - MATSEQDENSECUDA = "seqdensecuda" - A matrix type to be used for sequential
...
{
  PetscFunctionBegin;
  PetscCall(MatSolverTypeRegister_DENSECUPM<DeviceType::CUDA>());
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
{
  PetscFunctionBegin;
  PetscCall(MatMatMultNumeric_SeqDenseCUPM_SeqDenseCUPM<DeviceType::CUDA>(A, B, C, TA, TB));
  PetscFunctionReturn(PETSC_SUCCESS);
}

PetscErrorCode MatSeqDenseCUDAInvertFactors_Private(Mat A)
{
  PetscFunctionBegin;
