...
static constexpr MatSeqDense_CUPM<DeviceType::HIP> cupm_mat{};

/*MC
  MATSEQDENSEHIP - MATSEQDENSEHIP = "seqdensehip" - A matrix type to be used for sequential
...
{
  PetscFunctionBegin;
  PetscCall(MatSolverTypeRegister_DENSECUPM<DeviceType::HIP>());
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
{
  PetscFunctionBegin;
  PetscCall(MatMatMultNumeric_SeqDenseCUPM_SeqDenseCUPM<DeviceType::HIP>(A, B, C, TA, TB));
  PetscFunctionReturn(PETSC_SUCCESS);
}

PetscErrorCode MatSeqDenseHIPInvertFactors_Private(Mat A)
{
  PetscFunctionBegin;
