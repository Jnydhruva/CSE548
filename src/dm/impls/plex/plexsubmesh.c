...
static PetscErrorCode DMPlexSubmeshGetDimension_Static(DM dm, DM subdm, PetscInt *subdepth)
{
  ...
  PetscCallMPI(MPI_Allreduce(&d, subdepth, 1, MPIU_INT, MPI_MAX, PetscObjectComm((PetscObject)subdm)));
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
- addOverlap      - If PETSC_TRUE, add overlap using dm's adjacency rule and overlap size; only significant if ignoreLabelHalo is PETSC_TRUE
...
.seealso: `DMPlexGetSubpointMap()`, `DMGetLabel()`, `DMLabelSetValue()`, `DMPlexCreateSubmesh()`, `DMPlexDistributeOverlap()`
@*/
PetscErrorCode DMPlexFilter(DM dm, DMLabel cellLabel, PetscInt value, PetscBool ignoreLabelHalo, PetscBool addOverlap, DM *subdm)
{
  ...
  if (ignoreLabelHalo && addOverlap) {
    DM       osubdm;
    PetscSF  osf, ownershipTransferSF;
    PetscInt overlap;
...
. subpointIS - The IS of all the points from the original mesh in this submesh, or NULL if this is not a submesh
  ...
  Note: This IS is guaranteed to be sorted by the construction of the submesh if the submesh dimension equals to the parent mesh dimension
