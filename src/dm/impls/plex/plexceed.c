...
/*@C
  DMPlexGetLocalOffsetsFEM - Allocate and populate array of local offsets for each cell closure.
...
.seealso: [](chapter_unstructured), `DMPlexGetLocalOffsetsFVM()`, `DM`, `DMPLEX`, `DMLabel`, `DMPlexGetClosureIndices()`, `DMPlexSetClosurePermutationTensor()`, `DMPlexGetCeedRestriction()`
@*/
PetscErrorCode DMPlexGetLocalOffsetsFEM(DM dm, DMLabel domain_label, PetscInt label_value, PetscInt height, PetscInt dm_field, PetscInt *num_cells, PetscInt *cell_size, PetscInt *num_comp, PetscInt *l_size, PetscInt **offsets)
{
  PetscDS         ds = NULL;
  PetscFE         fe;
...
/*@C
  DMPlexGetLocalOffsetsFVM - Allocate and populate arrays of local offsets for each face support.
...
.seealso: [](chapter_unstructured), `DMPlexGetLocalOffsetsFEM()`, `DM`, `DMPLEX`, `DMLabel`, `DMPlexGetClosureIndices()`, `DMPlexSetClosurePermutationTensor()`, `DMPlexGetCeedRestriction()`
@*/
PetscErrorCode DMPlexGetLocalOffsetsFVM(DM dm, DMLabel domain_label, PetscInt label_value, PetscInt *num_faces, PetscInt *num_comp, PetscInt *l_size, PetscInt **offsetsNeg, PetscInt **offsetsPos)
{
  PetscDS         ds = NULL;
  PetscFV         fv;
  PetscSection    section;
  PetscInt        dim, height = 1, dm_field = 0, ds_field = 0, cell_size;
  ...
  PetscCall(DMGetPoints_Private(dm, domain_label, label_value, height, &iter_is));
  if (iter_is) {
    PetscCall(ISGetLocalSize(iter_is, num_faces));
    PetscCall(ISGetIndices(iter_is, &iter_indices));
  } else {
    *num_faces   = 0;
    iter_indices = NULL;
  ...
  cell_size = *num_comp;
  PetscInt restr_size = (*num_faces) * cell_size;
  ...
  for (PetscInt p = 0; p < *num_faces; ++p) {
...
PetscErrorCode DMPlexGetCeedRestriction(DM dm, DMLabel domain_label, PetscInt label_value, PetscInt height, PetscInt dm_field, CeedElemRestriction *ERestrict)
{
  ...
  if (!dm->ceedERestrict) {
    ...
    PetscCall(DMPlexGetLocalOffsetsFEM(dm, domain_label, label_value, height, dm_field, &num_cells, &cell_size, &num_comp, &lvec_size, &restr_indices));
